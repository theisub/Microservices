...
export default class PlanesList extends Component {  
  ...
  constructor(props) {  
    ...
    onChange = async (page) => {
      ...
      debugger;
      await axios.get('https://localhost:44375/api/planesgateway/companies/FrenchTour?pageSize='+this.state.pageSize+'&pageNum='+this.state.current)  
        .then(response => {  
          this.setState({ business: response.data, dataCount: response.data.length});  
          //debugger;  
        })  
    ...
    tabRow(){  
      return this.state.business.map(function(object, i){  
          return <PlaneTable obj={object} key={i}  />;  
      });  
    }  
    ...
    render() {  
      return (  
        <div>  
          <h4 align="center">Planes List</h4>  
